...
<template>
  <aside class="sidebar panel">
    ...
    <div v-if="data.martingale?.missing_platforms?.length || data.risk_management?.missing_platforms?.length" class="status-box warn"><strong>当前平台快照不完整</strong><span>缺少平台：{{ [...new Set([...(data.martingale?.missing_platforms || []), ...(data.risk_management?.missing_platforms || [])])].join('、') }}；请先在终端运行 scripts/refresh_local_data.py。</span></div>
