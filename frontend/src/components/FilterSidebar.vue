...
<template>
  <aside class="sidebar panel">
    ...
    <label class="wide">最低活跃交易日<input v-model.number="request.rules.min_active_days" type="number" min="0"></label>
    <label class="wide">最大杠杆率 P95<input v-model.number="request.rules.max_leverage_p95_ratio" type="number" min="0"></label>
    <label class="wide">最低盈亏比地板<input v-model.number="request.rules.min_payoff_ratio" type="number" min="0" step="0.01"></label>
    ...
    <div class="filter-group"><strong>核心规则</strong><label>最低胜率<input v-model.number="request.rules.min_win_rate" type="number" min="0" max="1" step="0.01"></label><label>最低 Profit Factor<input v-model.number="request.rules.min_profit_factor" type="number" min="0" step="0.01"></label><small>有效盈亏比 = max(地板, 联动系数×(1-胜率)/胜率)；中等严格默认联动 1.0，兼容高低胜率策略。</small></div>
    <div class="filter-group"><strong>持续性</strong><label>最低盈利月占比<input v-model.number="request.rules.min_positive_month_rate" type="number" min="0" max="1" step="0.01"></label><label>日盈利率 Wilson 下限<input v-model.number="request.rules.min_direction_day_rate_lower_bound" type="number" min="0" max="1" step="0.01"></label><label class="check"><input v-model="request.rules.require_selection_monthly_positive" type="checkbox"> 筛选期每月净盈利</label><label class="check"><input v-model="request.rules.require_selection_net_positive" type="checkbox"> 筛选期总净盈利 &gt; 0</label></div>
    <div class="filter-group"><strong>R4 独立通道（仅 5–6 月）</strong><label class="check"><input v-model="request.rules.enable_r4" type="checkbox"> 启用 R4 Abook 通道</label><label>连续通过周数<input v-model.number="request.rules.r4_min_passing_weeks" type="number" min="1" max="2"></label><small>运行阈值：胜率 ≥ 55%、主战场占比 ≥ 35%、交易数 ≥ 20、主战场交易数 ≥ 8、软对齐。</small></div>
    ...
    <div v-if="data.martingale?.missing_platforms?.length || data.risk_management?.missing_platforms?.length || data.r4?.missing_platforms?.length || data.avg_profit?.missing_platforms?.length" class="status-box warn"><strong>当前平台快照不完整</strong><span>缺少平台：{{ [...new Set([...(data.martingale?.missing_platforms || []), ...(data.risk_management?.missing_platforms || []), ...(data.r4?.missing_platforms || []), ...(data.avg_profit?.missing_platforms || [])])].join('、') }}；请点击“刷新全部数据”后再应用筛选，否则该平台不会有完整的马丁、杠杆、R4 和盈利快照保护。</span></div>
