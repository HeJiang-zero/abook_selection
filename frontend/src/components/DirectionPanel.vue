...
import { useChartList } from '../composables/useChart'
...
// Local editable copy — props.request is read-only.
...
const passPageSize = 20
const cumulativeCharts = ref<HTMLElement[]>([])
const pnlDistributionCharts = ref<HTMLElement[]>([])
const cumulativeChartList = useChartList()
const distributionChartList = useChartList()
...
function flagLabel(flag: string): string {
  const labels: Record<string, string> = {
    ...
    long_trades_ratio: '多单比例未达标', profit_concentration: 'Top1 日集中度过高',
    leverage_p95_ratio: '杠杆未通过', martingale_hard_block: '马丁硬拦截',
  }
  return labels[flag] || flag
}
function accountFlags(account: Record<string, any>): string[] {
  return [...(account.selection_flags_long || []), ...(account.selection_flags_short || [])]
}
...
const passPageCount = computed(() => Math.max(1, Math.ceil(selectedPassAccounts.value.length / passPageSize)))
const pagedPassAccounts = computed(() => selectedPassAccounts.value.slice((passPage.value - 1) * passPageSize, passPage.value * passPageSize))
...
const filteredRows = computed(() => {
  ...
  return all.filter(account => {
    ...
    return String(account.login).includes(query) || String(account.platform).toLowerCase().includes(query)
  })
})
...
function renderCumulativeCharts() {
  sides.forEach((which, index) => {
    ...
    cumulativeChartList.render(element, {
...
function renderPnlDistributionCharts() {
  ...
  sides.forEach((which, index) => {
    ...
    distributionChartList.render(element, {
      backgroundColor: 'transparent',
...
// Split: cumulative depends on book/set (not phase); distribution depends on phase too.
watch([activeBook, activeSet], () => {
  selectedPassSet.value = null
  passPage.value = 1
  nextTick(renderCumulativeCharts)
})
watch([activeBook, activeSet, phase], () => nextTick(renderPnlDistributionCharts))
...
watch(selectedPassSet, () => { passPage.value = 1 })
</script>
...
<template>
  <section class="panel direction-panel">
    <div class="panel-head">
      ...
      <div class="direction-actions"><button class="primary" :disabled="loading || !localRequest.platforms.length" @click="runSearch">{{ analytics ? '重新运行分向筛选' : '运行分向筛选' }}</button><button class="ghost" :disabled="!analytics || loading" @click="emit('export')">导出分向 CSV</button></div>
    </div>
    <div class="direction-parameters">
      ...
      <div class="direction-parameter-footer">
        <div class="platforms"><label v-for="platform in ['mt4', 'mt5', 'hh_mt5']" :key="platform" class="check"><input v-model="localRequest.platforms" :value="platform" type="checkbox"> {{ platform }}</label></div>
        <div class="direction-martingale"><span>确认马丁阻断等级</span><label v-for="level in martingaleLevels" :key="level" class="check"><input v-model="localRequest.rules.excluded_martingale_levels" type="checkbox" :value="level"> {{ level }}</label><small>当前选中的确认马丁等级会被分到 Bbook。</small></div>
      </div>
    </div>
