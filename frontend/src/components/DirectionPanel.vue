...
import type { DirectionAnalyticsPayload } from '../types'

const props = defineProps<{ analytics: DirectionAnalyticsPayload | null; loading: boolean }>()
...
function flagLabel(flag: string): string {
  const labels: Record<string, string> = {
    ...
    profit_concentration: 'Top1 日集中度过高', leverage_p95_ratio: '杠杆未通过', martingale_hard_block: '马丁硬拦截',
  }
...
function phaseData(book: Book, which: Side): any {
  return payloadFor(book, passSet(which))[which]?.phase_summary?.[phase.value] || {}
}
...
const rows = computed(() => filteredRows.value.filter(account => {
  if (activeSet.value !== 'all') return true
  const metric = account[side.value]?.[phase.value] || {}
  return Math.abs(number(metric.side_pnl)) > 10
}).sort((left, right) => {
...
function renderCumulativeCharts() {
  ...
  sides.forEach((which, index) => {
    ...
    if (!element) return
    const setName = passSet(which)
    const dataByBook = Object.fromEntries(books.map(book => [book, payloadFor(book, setName)[which]?.cumulative_pnl?.full || []])) as Record<Book, any[]>
    const dates = [...new Set(books.flatMap(book => dataByBook[book].map(row => row.date)))].sort()
    ...
    chart.setOption({
      ...
      series: books.map(book => {
        ...
        return { name: bookLabel(book), type: 'line', smooth: true, showSymbol: false, data: dates.map(date => values[date] ?? null), connectNulls: false, lineStyle: { width: 3, color: book === 'abook' ? '#54d6a6' : '#ff8d91' }, areaStyle: { color: book === 'abook' ? 'rgba(84,214,166,.10)' : 'rgba(255,141,145,.08)' } }
      }),
    })
...
function renderPnlDistributionCharts() {
  ...
  sides.forEach((which, index) => {
    ...
    if (!element) return
    const setName = passSet(which)
    const dataByBook = Object.fromEntries(books.map(book => [book, payloadFor(book, setName)[which]?.pnl_distribution?.[phase.value] || []])) as Record<Book, any[]>
    const rowsByKey = Object.fromEntries((dataByBook.abook.length ? dataByBook.abook : dataByBook.bbook).map(row => [row.key, row]))
    ...
    chart.setOption({
      ...
      series: books.map(book => ({ name: bookLabel(book), type: 'bar', barMaxWidth: 30, data: Object.keys(rowsByKey).map((key, bucketIndex) => ({ value: dataByBook[book].find(row => row.key === key)?.accounts || 0, itemStyle: { color: book === 'abook' ? colors[bucketIndex] : `${colors[bucketIndex]}99` } })) })),
    })
    pnlDistributionInstances.push(chart)
  })
}
watch([() => props.analytics, activeSet, phase, side], () => nextTick(() => {
