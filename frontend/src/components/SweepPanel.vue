...
<template>
  <section class="panel">
    ...
    <div class="table-scroll"><table><thead><tr><th>Abook Core</th><th>继续盈利率</th><th>验证期公司增量（理论）</th><th>误判成本（公司少赚）</th><th>净公司收益（理论）</th><th>Lift</th><th>样本</th></tr></thead><tbody><tr v-for="row in results" :key="JSON.stringify(row.rules)" class="clickable" @click="emit('applyRules', row.rules)"><td>{{ row.abook_core }}</td><td>{{ (row.continue_profit_rate * 100).toFixed(1) }}%</td><td>{{ row.validation_increment.toFixed(2) }}</td><td>{{ row.misjudge_cost.toFixed(2) }}</td><td>{{ row.net_gain.toFixed(2) }}</td><td>{{ row.lift.toFixed(2) }}</td><td :class="row.sample_warning ? 'warning' : ''">{{ row.validation_active_accounts }}{{ row.sample_warning ? ' ⚠' : '' }}</td></tr></tbody></table><div v-if="!results.length" class="empty">定义网格后开始扫描</div></div>
  </section>
</template>
