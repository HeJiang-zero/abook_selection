...
import type { BookAnalyticsPayload, RequestModel } from '../types'

const props = defineProps<{ analytics: BookAnalyticsPayload | null; loading: boolean; activeTab: string; request: RequestModel }>()
    ...
    periods.forEach((period, periodIndex) => {
      ...
      chart.setOption({
        ...
        series: [{
          ...
          data: periodRows.map((row, index) => ({ value: row.accounts, itemStyle: { color: distributionBuckets[index].color } })),
        }],
      })
