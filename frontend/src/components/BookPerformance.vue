...
async function renderChart() {
  ...
  if (props.activeTab === 'risk') {
    const abook = analytics.risk_exposure?.abook?.leverage_distribution || []
    const bbook = analytics.risk_exposure?.bbook?.leverage_distribution || []
    x = Array.from({ length: Math.max(abook.length, bbook.length) }, (_, index) => `用户${index + 1}`)
    series = [
      { name: 'Abook 杠杆率 P95', type: 'line', smooth: true, data: abook, lineStyle: { color: '#54d6a6' } },
      { name: 'Bbook 杠杆率 P95', type: 'line', smooth: true, data: bbook, lineStyle: { color: '#ff8d91' } },
    ]
