...
async function renderChart() {
  ...
  if (props.activeTab === 'risk') {
    ...
    const byDate = (rows: any[]) => Object.fromEntries(rows.map(row => [row.date, row.cumulative_pnl]))
    const am = byDate(a); const bm = byDate(b)
    series = [
      { name: 'Abook 累计 P&L', type: 'line', data: x.map(date => am[date] ?? 0), lineStyle: { color: '#54d6a6' }, areaStyle: { color: 'rgba(84,214,166,.12)' } },
      { name: 'Bbook 累计 P&L', type: 'line', data: x.map(date => bm[date] ?? 0), lineStyle: { color: '#ff8d91' } },
    ]
  }
...
<template>
  <section class="panel">
    ...
    <template v-else-if="analytics">
      <div class="book-grid"><article v-for="book in ['abook', 'bbook']" :key="book" class="book-card"><h3>{{ book === 'abook' ? 'Abook' : 'Bbook（Core + observation）' }}</h3><p>筛选期 P&amp;L <b>{{ (analytics.pnl_structure?.[book]?.selection?.total_client_net_pnl ?? 0).toFixed(2) }}</b></p><p>验证期 P&amp;L <b>{{ (analytics.pnl_structure?.[book]?.validation?.total_client_net_pnl ?? 0).toFixed(2) }}</b></p><p>最大回撤 <b>{{ (analytics.pnl_structure?.[book]?.max_drawdown ?? 0).toFixed(2) }}</b></p><p>Top 5 集中度 <b>{{ ((analytics.pnl_structure?.[book]?.profit_concentration?.top_5?.absolute_share ?? 0) * 100).toFixed(1) }}%</b></p><p>验证 Turnover <b>{{ (analytics.risk_exposure?.[book]?.validation_turnover ?? 0).toFixed(2) }}</b></p></article></div>
      <div ref="chart" class="book-chart"></div>
      <div v-if="activeTab === 'users'" class="two-col metric-columns"><article v-for="book in ['abook', 'bbook']" :key="book" class="book-card"><h3>{{ book }} 用户结构</h3><div v-for="style in analytics.user_structure?.[book]?.styles || []" :key="style.style" class="list-row"><span>{{ style.style }}</span><b>{{ style.accounts }} 人 / {{ style.validation_client_net_pnl.toFixed(2) }}</b></div><h4>品种盈亏热力图</h4><div v-for="row in analytics.user_structure?.[book]?.symbol_heatmap || []" :key="row.symbol" class="list-row"><span>{{ row.symbol }}</span><b>{{ row.market_pnl.toFixed(2) }} · {{ row.trade_count }}</b></div></article></div>
