...
import { money, pct, pnlClass, profitableRate } from '../utils/format'
...
const riskChartList = useChartList()
const pnlChartList = useChartList()
const distributionChartList = useChartList()
const symbolChartList = useChartList()
...
function monthRangeLabel(start: string, end: string): string {
  ...
  const fmt = (ym: string) => `${Number(ym.slice(5))}月`
  return startMonth === endMonth ? fmt(startMonth) : `${fmt(startMonth)}–${fmt(endMonth)}`
}
function phaseLabel(phase: string): string {
  if (phase === 'selection') {
    const label = monthRangeLabel(props.request.selection.start, props.request.selection.end)
    return label ? `筛选期（${label}）` : '筛选期'
  }
  const label = monthRangeLabel(props.request.validation.start, props.request.validation.end)
  return label ? `验证期（${label}）` : '验证期'
}
function selectionTotalLabel(): string {
  const label = monthRangeLabel(props.request.selection.start, props.request.selection.end)
  return label ? `${label}合计` : '筛选期合计'
}
const distributionPeriods = computed(() => {
  ...
  return [...periods].map(period => ({
    key: period,
    label: period === 'selection_total' ? selectionTotalLabel() : period,
  }))
})
...
function renderSymbolCharts(analytics: BookAnalyticsPayload) {
  books.forEach((book, index) => {
    ...
    const chartRows = rows
      ...
      .slice(0, 20)
    symbolChartList.render(element, {
...
function renderDistributionCharts(analytics: BookAnalyticsPayload) {
  ...
  books.forEach((book, bookIndex) => {
    ...
    periods.forEach((period, periodIndex) => {
      ...
      const neutralIndex = distributionBuckets.findIndex(bucket => bucket.key === 'neutral')
        ...
        series: [
          {
            ...
            data: periodRows.map((row, index) => index === neutralIndex ? null : { value: row.accounts, itemStyle: { color: distributionBuckets[index].color } }),
          },
          {
            name: '中性/无效用户数',
            type: 'bar',
            ...
            barMaxWidth: 42,
            label: { show: true, position: 'top', color: '#9fb3c9' },
            data: periodRows.map((row, index) => index === neutralIndex ? { value: row.accounts, itemStyle: { color: distributionBuckets[index].color } } : null),
          },
        ],
      })
    })
...
function renderPnlCharts(analytics: BookAnalyticsPayload) {
  ...
  pnlChartList.render(cumulativeChart.value, {
    backgroundColor: 'transparent',
  ...
  pnlChartList.render(dailyChart.value, {
    backgroundColor: 'transparent',
    tooltip: { trigger: 'axis' },
    legend: { textStyle: { color: '#9fb3c9' } },
    grid: { left: 96, right: 96, top: 42, bottom: 28, containLabel: true },
...
function renderRoutingChart(analytics: BookAnalyticsPayload) {
  ...
  riskChartList.render(routingChart.value, {
    backgroundColor: 'transparent',
    tooltip: { trigger: 'axis' },
    legend: { textStyle: { color: '#9fb3c9' } },
...
async function renderChart() {
  ...
  riskChartList.render(chart.value, {
    backgroundColor: 'transparent',
    tooltip: { trigger: 'axis' },
    legend: { textStyle: { color: '#9fb3c9' } },
...
watch(() => [props.analytics, props.activeTab], renderChart, { immediate: true })
</script>
