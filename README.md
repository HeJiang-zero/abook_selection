...
当前 Abook 利润影响是理论估算：假设进入 Abook 后交易所的对手盘利润为 0，因此迁移增量为用户净交易 P&L；不包含真实外部成交、点差、对冲成本、滑点和流动性成本。所有用户只有 Abook/Bbook 两种归属。
...
杠杆和马丁快照不在每次页面请求中重新聚合。统一使用独立 CLI 刷新 Warehouse 和两个快照：
