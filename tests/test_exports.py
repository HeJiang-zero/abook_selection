...
def _account(login: int, cohort: str, selection_pnl: float, validation_pnl: float, source: str = "rule_filter"):
    return {
        "platform": "mt5", "login": login, "account_group": "real",
        "cohort": cohort, "selection_source": source, "selection_client_net_pnl": selection_pnl,
        ...
        "base_cohort": "abook_candidate", "stability": {"tier": "core", "score": 85},
...
def test_misjudge_cost_is_sum_of_validation_losses_only():
    summary = build_misjudge_summary([
        _account(1, "abook_candidate", 50, -40),
        _account(2, "abook_candidate", 40, 30),
        _account(3, "bbook_candidate", -40, 25),
    ])
...
def test_funnel_reports_ordered_stages_and_drop_reasons():
    ...
    assert [stage["name"] for stage in funnel["stages"]] == [
        "eligible", "sample_qualified", "directional", "stability_core",
        "leverage_passed", "non_martingale", "abook_core",
    ]
...
def test_export_contains_only_final_abook_and_utf8_bom():
    csv_text = render_abook_csv([_account(7, "abook_candidate", 50, 20)])

    assert csv_text.startswith("\ufeff")
    assert "platform,login" in csv_text
    assert "mt5,7" in csv_text

    bbook_csv = render_abook_csv([_account(8, "bbook_candidate", -20, 20)])
